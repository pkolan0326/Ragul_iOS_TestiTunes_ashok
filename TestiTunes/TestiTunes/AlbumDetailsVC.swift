...
class AlbumDetailsVC: UIViewController {
    ...
    func addControls() {
        ...
        linkButton.backgroundColor = UIColor.appTheme()
    ...
    {
        let albumID = selectedAlbum.albumID
        if  let url = URL.init(string: selectedAlbum.artworkUrl100!) {
            albumImageView.downloadImage(from: url, UIImage.init(named: "audio"), albumID!)
        }
        nameLbl.text = selectedAlbum.name
        artistValueLbl.text = selectedAlbum.artistName
...
//       if let genNames = geners.map({$0["name"]}) as? [String] {
       ...
       //                obj.genres = info
       //            }
        self.genreInfoLbl.text = selectedAlbum.genres?.map{$0.name!}.joined(separator : ", ")
        //Release Date
        releaseDateLbl.text = selectedAlbum.releaseDate
        //CopyRight
        copyRightLbl.text = selectedAlbum.copyright
   ...
   /* func loadAlbumData(){
    ...
    @objc func linkBtnTapped(_ btn: UIButton){
        if  let url = URL.init(string: selectedAlbum.artistURL!)
        {
...
extension UIColor {
   class func appTheme() -> UIColor {
        return UIColor.init(red: CGFloat(70.0/255.0), green: CGFloat(160.0/255.0), blue: CGFloat(240.0/255.0), alpha: 1.0)
    }
}
